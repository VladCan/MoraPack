...
public final class ImpresorSolucion {
    ...
    // ========= Helpers =========
    private static String ts(Instant t) { return (t == null ? "-" : TS_SHORT.format(t)); }
    ...
    private static String formatearReporte(SolucionProgramacion sol) {
        ...
        sb.append(String.format("Capacidades vuelos: %s  |  SLA llegada≤46h (todos): %s  |  SLA 48h (todos): %s%n",
        ...
        String header = String.format(
                ...
                "ID","DST","CREADO","DEMANDA","ASIGN","%ASIG","1a_LLEG","ÚLT_LLEG","≤46h","SLA48","PROGRESO");
        sb.append(header);
        ...
        final String subHeader = String.format("      %-9s %-4s %-4s %-13s %-13s %6s%n",
        ...
        sb.append(hv).append("-".repeat(hv.length())).append('\n');

        var usados = sol.getCargaPorVuelo().getAsignado().entrySet().stream()
                .filter(e -> e.getValue() != null && e.getValue() > 0)
                .sorted(Comparator
                        .comparing((Map.Entry<VueloProgramadoId, Integer> e) -> e.getKey().getSalidaUtc())
