...
public class SolucionProgramacion {
    ...
    public boolean respetaSLAConPickupTodos(Duration pickupTime) {
        ...
        return planPorPedido.values().stream().allMatch(p -> {
            ...
            if (ult == null)
                return false; // sin llegadas, no cumple
    ...
    public static final class FitnessResult {
        ...
        public final long vuelos;
        public final long capTotal;
        public final long asignadoClamped;
        public final long capOverUnits;
        public final int capOverVuelos;
        public final long slaLateHours;
        public final int slaLatePedidos;
        ...
        FitnessResult(
            ...
            this.U = U;
            this.C = C;
            this.splitsTotales = splitsTotales;
            this.vuelos = vuelos;
            this.capTotal = capTotal;
            this.asignadoClamped = asignadoClamped;
            this.capOverUnits = capOverUnits;
            this.capOverVuelos = capOverVuelos;
            this.slaLateHours = slaLateHours;
            this.slaLatePedidos = slaLatePedidos;
            this.termT = termT;
            this.termU = termU;
            this.termC = termC;
            this.termS = termS;
    ...
    public void imprimirCapacidadVuelosEnVentana(Instant inicio, Instant fin, String nombreArchivo) {
        ...
        for (var e : cargaPorVuelo.getAsignado().entrySet()) {
            ...
            if (sal == null || sal.isBefore(inicio) || !sal.isBefore(fin))
                continue; // filtro por ventana
        ...
        java.nio.file.Path path = java.nio.file.Path.of(
                (nombreArchivo == null || nombreArchivo.isBlank())
                        ? "out/reporteCapacidadVuelos.txt"
                        : nombreArchivo);
        ...
        try {
            ...
            try (var bw = java.nio.file.Files.newBufferedWriter(
                ...
                for (Row r : filas) {
                    ...
                    if (over)
                        estado = estado + "  **EXCESO +" + (r.asg - r.cap) + "**";
    ...
    private static String obtenerSaturacion(int capacidad, int ocupacion) {
        ...
        if (pct < 33.33)
            return "Disponible ✅ " + pctRed + "%";
        else if (pct < 66.66)
            return "Limitado ⚠️ " + pctRed + "%";
        else if (pct < 99.99)
            return "Saturado ❌ " + pctRed + "%";
        else if (pctRed == 100)
            return "Capacidad máxima 🚫🚫🚫";
        else
            return "Exceso total ☠️☠️☠️";
    }
