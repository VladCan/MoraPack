...
public final class ImpresorSolucion {
        ...
        private static final DateTimeFormatter ID_SALIDA = DateTimeFormatter.ofPattern("yyyyMMdd'T'HHmmss'Z'")
                        .withZone(ZoneOffset.UTC);
        ...
        private static String vueloIdStr(VueloProgramadoId v) {
                ...
                String s = v.getSalidaUtc() == null ? "" : ID_SALIDA.format(v.getSalidaUtc());
                return o + d + s;
        }
        ...
        private static final Duration PICKUP_TIME = Duration.ofHours(2);
        ...
        // ===== API principal =====
        public static void imprimirEnConsola(SolucionProgramacion sol, Instant presenteUTC) {
                System.out.println(formatearReporte(sol, presenteUTC));
        }
        ...
        public static Path guardarTodo(SolucionProgramacion sol, Instant presenteUtc, String prefijo) {
                try {
                        ...
                        Files.writeString(txt, formatearReporte(sol, presenteUtc), StandardCharsets.UTF_8,
                        ...
                        for (RutaAsignada ruta : plan.getRutas()) {
                                ...
                                for (int i = 0; i < tramos.size(); i++) {
                                        ...
                                        if (i == 0) {
                                                ...
                                                esperaInicioOrigen = (anterior != null) ? anterior.getLlegadaUtc()
                                                                : null;
                                        }
                                        ...
                                        if (esperaInicioOrigen != null && esperaFinOrigen != null
                                                ...
                                                origen.periodos.add(new PeriodoBodega(esperaInicioOrigen,
                                                                esperaFinOrigen, cantidad));
                                        }

                                        Instant esperaInicioDestino = vuelo.getLlegadaUtc();
                                        Instant esperaFinDestino = null;
                                        if (i + 1 < tramos.size()) {
                                                VueloProgramadoId siguienteVuelo = tramos.get(i + 1).getVuelo();
                                                esperaFinDestino = (siguienteVuelo != null)
                                                                ? siguienteVuelo.getSalidaUtc()
                                                                : null;
                                        } else {
                                                // Cliente final: mantener en bodega hasta el tiempo de pickup
                                                esperaFinDestino = (esperaInicioDestino != null)
                                                                ? esperaInicioDestino.plus(PICKUP_TIME)
                                                                : null;
                                        }

                                        if (esperaInicioDestino != null && esperaFinDestino != null
                                                        && !esperaFinDestino.isBefore(esperaInicioDestino)) {
                                                destino.periodos.add(new PeriodoBodega(esperaInicioDestino,
                                                                esperaFinDestino, cantidad));
                                        }
                                }
                        }
                }
        ...
        private static String formatearReporte(SolucionProgramacion sol, Instant presenteUtc) {
                ...
                // sb.append(String.format("Fecha y hora de ejecución: %s%n",
                // fmt(presenteUtc)));
                ...
                // Subtabla de rutas (ahora con VUELO_ID y PREV_ID)
                int[] rutasCols = { 12, 5, 5, 16, 16, 24, 24, 8 };
                String hRutas = "      " + row(
                                rutasCols,
                                "RUTA(q)", "ORI", "DST", "SALE", "LLEGA", "VUELO_ID", "PREV_ID", "ASIG");
                String rRutas = "      " + rule(rutasCols);
                        ...
                        if (rutas != null && !rutas.isEmpty()) {
                                ...
                                for (RutaAsignada r : rutas) {
                                        ...
                                        for (int i = 0; i < tramos.size(); i++) {
                                                ...
                                                VueloProgramadoId prev = (i > 0 ? tramos.get(i - 1).getVuelo() : null);
                                                ...
                                                sb.append("      ").append(row(
                                                                ...
                                                                vueloIdStr(v),
                                                                vueloIdStr(prev), // vacío si no hay predecesor
